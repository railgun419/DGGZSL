...
parser.add_argument('--data', '-d', metavar='DATA', default='cub',
                    help='dataset')
parser.add_argument('--xlsa17', metavar='DATA', default='../../ZSL_Dataset/xlsa17/data/',
                    help='dataset')
...
parser.add_argument('--cluster_K', default=1, type=int, help='')
parser.add_argument('--m', default=0.10, type=float, help='margin in circle loss')
parser.add_argument('--gamma', default=10, type=float, help='scale factor in circle loss')
parser.add_argument('--K', default=30, type=int, help='select K positive attributes for contrastive loss')
parser.add_argument('--T', default=0.5, type=float, help='temperature in contrastive loss')
parser.add_argument('--w_L_cls', default=1.0, type=float, help='')
parser.add_argument('--w_L_zsl', default=1.0, type=float, help='')
parser.add_argument('--w_L_v2s', default=300.0, type=float, help='')
parser.add_argument('--w_L_ml', default=0.1, type=float, help='')
parser.add_argument('--w_L_mask', default=1.0, type=float, help='')
''' opt for others '''
parser.add_argument('--save-path', '-s', metavar='SAVE', default='',
                    help='saving path')
parser.add_argument('--resnet_pretrain', default='../APN/pretrained_models/resnet101-5d3b4d8f.pth',
...
def main_worker(ngpus_per_node, args):
    ...
    args.seen_c = semantic_data['seen_class']
    args.unseen_c = semantic_data['unseen_class']
    # todo: cluster for domains
    args.domain_list = cluster(args)
    ...
    # train_sampler = torch.utils.data.distributed.DistributedSampler(train_dataset, num_replicas=num_tasks,
    ...
    criterions = [nn.CrossEntropyLoss().cuda(),
                  nn.MSELoss(reduction='none').cuda(),
                  ]
...
def train(train_loader, semantic_data, model, criterions, od_optimizer, zs_optimizer, optimizer, epoch, args):
    ...
    Loss_v2s = AverageMeter('L_v2s', ':.4f')
    Loss_ml = AverageMeter('L_ml', ':.4f')
    Losses.append(Loss_s2v)
    Losses.append(Loss_cls)
    Losses.append(Loss_v2s)
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # compute output
        logit_zsl, logit_cls, logit_ood, pre_attri, att_weight, loss_ml, loss_sp, loss_iou, visual_info = model(input, target)
        # zsl loss
        L_s2v = 0
        ...
        Loss_ml.update(L_ml)
        # update
        total_loss = args.w_L_zsl * L_s2v + args.w_L_cls * L_cls + args.w_L_v2s * L_v2s + args.w_L_ml * L_ml
        if 'ood' in args.model:
            # ood loss
    ...
    with torch.no_grad():
        ...
        for i, (input, target) in enumerate(val_loader2):
            ...
            if (i == 0):
                ...
                # zsl_logit_t1 = np.vstack([zsl_logit_t1, logit_v2s])
        ...
        for th_pp in np.arange(a, b, c):
            zsl_logit_s2 = zsl_logit_s.copy()
            zsl_logit_t2 = zsl_logit_t.copy()
            zsl_logit_s2[:, seen_c] = zsl_logit_s2[:, seen_c] * th_pp
            zsl_logit_t2[:, seen_c] = zsl_logit_t2[:, seen_c] * th_pp
            ST = compute_class_accuracy_total(gt_s, np.argmax(zsl_logit_s2, axis=1), seen_c)
            UT = compute_class_accuracy_total(gt_t, np.argmax(zsl_logit_t2, axis=1), unseen_c)
            H = 2 * ST * UT / (ST + UT)
            if H > H1:
                H1 = H
                S1 = ST
        ...
        if 'ood' in args.model:
            zsl_logit[:, seen_c] = -1
            opt_H, opt_S, opt_U, opt_Ds, opt_Du, opt_tau = ood_opt(ood_logit, zsl_logit, gt, gt_s.shape[0], seen_c,
                                                                   unseen_c)
            if opt_H > H1:
                H1 = opt_H
                S1 = opt_S
